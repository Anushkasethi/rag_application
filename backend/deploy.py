...
pip3 install fastapi uvicorn langchain sentence-transformers faiss-cpu beautifulsoup4 python-multipart rank_bm25 langchain-core nltk pdfplumber cohere python-dotenv psutil
...
def run_server(key_pair_name, public_dns_name, remote_path):
    ...
    os.system(
        f"ssh -o StrictHostKeyChecking=no -i {key_pair_name}.pem ubuntu@{public_dns_name} 'source {remote_path}/venv/bin/activate && nohup python3 {remote_path}/frontend/server.py > {remote_path}/server.log 2>&1 &' &"
    )
...
def _run_command(command):
    match command:
        ...
        case "--serve":
            ...
            server_status = os.system(f"ssh -o StrictHostKeyChecking=no -i {key_pair_name}.pem ubuntu@{public_dns_name} 'sudo lsof -i :8080' > /dev/null")
            if server_status == 0:
        ...
        case "--unserve":
            ...
            os.system(f"ssh -o StrictHostKeyChecking=no -i {key_pair_name}.pem ubuntu@{public_dns_name} 'sudo kill $(sudo lsof -t -i:8080)'")
        ...
        case "--describe":
            ...
            with open("instance_public_ip.txt", "r") as file:
                ...
                print(f"Search engine is running at http://{public_ip_address}:8080")
